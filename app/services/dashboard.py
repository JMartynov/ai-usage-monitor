...
async def get_total_usage(db: AsyncSession) -> Dict[str, Any]:
    ...
    total_tokens = result[1] if result and result[1] is not None else 0
    total_cost = result[2] if result and result[2] is not None else 0.0
...
async def get_token_breakdown(db: AsyncSession) -> Dict[str, int]:
...
async def get_recent_activity(
    ...
    recent_activity_query = await db.execute(
        ...
        ).order_by(desc(RequestLog.timestamp)).limit(limit)
    )
    return [
        {
            "id": row.id,
...
async def get_expensive_requests(
    ...
    expensive_requests_query = await db.execute(
        select(
            ...
            RequestLog.timestamp
        ).order_by(desc(RequestLog.estimated_cost)).limit(limit)
    )
    return [
        {
            "id": row.id,
